...
from my_package.utils import (
    COLOR_OK, COLOR_NOK, COLOR_BORDER, COLOR_TEXT,
...
def afficher_etat_services():
    ...
    services = ["wan", "lan", "gestion", "certif", "logs"]
    colored_data = []

    for service in services:
        etat = config[service]
        etat_txt = f"{GREEN}Activé{NC}" if etat else f"{RED}Désactivé{NC}"
        colored_data.append([f"{YELLOW}{service.upper()}{NC}", etat_txt])
...
def activ_desactiv():
    ...
    while True:
        os.system('clear')  # Nettoie l'écran à chaque boucle
        show_banner()
