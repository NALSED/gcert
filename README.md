...
# Carte d'interaction des scripts GCert

```mermaid
flowchart TD
    %% Shell
    GSH[g_cert.sh] --> LOAD[load.sh]

    %% Script principal
    MAIN[main.py] --> ACC[acces.py]
    MAIN --> GM[gestion_menu.py]
    MAIN --> MOD[modif.py]
    MAIN --> CERT[Certifs/certif.py]
    MAIN --> LAN[Lan/lan.py]
    MAIN --> WAN[Wan/wan.py]
    MAIN --> LOGM[Logs/logs_Menu.py]

    %% Gestion Menu
    GM --> GC[Gestion/Certif/gestion_certif.py]
    GM --> GP[Gestion/Pass/gestion_pass.py]

    %% Gestion Pass
    GP --> DK[Gestion/Key/delete_key.py]
    GP --> MM[Gestion/Pass/modif_mdp.py]
    GP --> EM[Gestion/Pass/etat_mdp.py]

    DK --> MMG[Gestion/Key/modif_mdp_gpg.py]
    MMG --> CH[Gestion/Pass/changement_mdp.py]

    EM --> CA[Gestion/Pass/config_auth.py]
    CA --> AUTH[script/auth_config.json]

    %% Logs
    LOGM --> LOGA[Logs/logs_Arch.py]
